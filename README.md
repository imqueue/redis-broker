...
no pull request. `master` is covered by a ruleset that requires the
`attribution / No AI attribution` check on every push, which a bot commit cannot
satisfy — so storing signatures there made every signature fail to record, and no
outside contribution could be merged. The action's own guidance is that the
signature branch must not be protected.
